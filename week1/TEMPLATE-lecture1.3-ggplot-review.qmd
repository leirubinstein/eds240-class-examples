...
```{r}
#| warning: false
# A default statistical transformation (count) is applied here
# Notice there is no count column in the original dataframe
...
#| warning: false
ggplot(penguins, aes(x = species, fill = island)) +
  geom_bar(position = position_dodge2(preserve = "single")) # makes all of the bars the same width
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  coord_cartesian() # don't need to include this unless you use arguments
```
...
ggplot(penguins, aes(x = species)) +
  geom_bar() +
  coord_flip() # could also redefine axes in aes()
```
